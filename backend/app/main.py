...
def compute_spectral_indices_for_point(lat: float, lon: float, use_live_telemetry: bool = False) -> tuple[dict, dict]:
    ...
    # Soil and climate indices from live telemetry or proximity
    live_soil = telemetry.get("soil_weathering_index")
    live_climate = telemetry.get("climate_weathering_index")
    soil_w = live_soil if live_soil is not None else round(0.65 + 0.65 * proximity_factor, 3)
    climate_w = live_climate if live_climate is not None else round(0.70 + 0.55 * proximity_factor, 3)
    ...
    features = {
        ...
        "soil_weathering_index": soil_w,
        "climate_weathering_index": climate_w,
    }
...
def scan_coordinates(req: CoordinateScanRequest):
    ...
    if req.soil_weathering_index is not None:
        features["soil_weathering_index"] = req.soil_weathering_index
    if req.climate_weathering_index is not None:
        features["climate_weathering_index"] = req.climate_weathering_index
    ...
    factors = [
        FeatureFactor(
            key="soil_weathering_index",
            label="Soil Weathering & Oxidation Index",
            value=features["soil_weathering_index"],
            status="optimal" if features["soil_weathering_index"] >= 1.15 else ("moderate" if features["soil_weathering_index"] >= 0.80 else "sub-optimal"),
            description="Oxidized lateritic soil profile conducive to supergene Mn oxide accumulation vs. barren alluvium/sand.",
        ),
        FeatureFactor(
            key="climate_weathering_index",
            label="Climate & Supergene Leaching Regime",
            value=features["climate_weathering_index"],
            status="optimal" if features["climate_weathering_index"] >= 1.15 else ("moderate" if features["climate_weathering_index"] >= 0.70 else "sub-optimal"),
            description="Tropical alternating wet-dry monsoon cycle driving silica leaching and secondary Mn concentration.",
        ),
        FeatureFactor(
            key="laterite_index",
    ...
    if prob >= 0.80:
        ...
        verdict = "Low Mineralization Signature (Incompatible / Barren)"
        rec = "Low priority for manganese exploitation. Pedological and climate indicators demonstrate non-mineralized or barren host conditions."
    ...
    return CoordinateScanResponse(
        ...
        soil_weathering_index=features.get("soil_weathering_index"),
        climate_weathering_index=features.get("climate_weathering_index"),
        soil_profile_desc=telemetry.get("soil_profile_desc"),
    )
